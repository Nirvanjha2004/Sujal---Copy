...
export function LeadManagementPage() {
  ...
  const fetchSiteVisits = async () => {
    try {
      setLoading(true);
      const visits = await leadService.fetchSiteVisits({ status: activeTab as any });
  ...
  const fetchSiteVisitStats = async () => {
    try {
      const statsData = await leadService.fetchSiteVisitStats();
      setStats(statsData);
    } catch (err) {
  ...
  const handleUpdateVisit = async () => {
    ...
    try {
      ...
      await leadService.updateSiteVisit(selectedVisit.id, {
