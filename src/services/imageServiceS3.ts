...
export class ImageServiceS3 {
  ...
  static async processAndSavePropertyImage(
    ...
    try {
      ...
      // Process image and generate all sizes in both JPEG and WebP formats
      let processedImages: any;
      let webpSizes: any = null;
      let uploadError: Error | null = null;

      try {
        // Try to process with WebP support
        processedImages = await imageProcessingService.processImageWithWebP(buffer, {
          quality,
          generateSizes: shouldGenerateSizes,
        });
        webpSizes = processedImages.webp;
        processedImages = processedImages.jpeg;
      } catch (webpError) {
        // Fallback to JPEG only if WebP processing fails
        console.warn('WebP processing failed, falling back to JPEG only:', webpError);
        processedImages = await imageProcessingService.processImage(buffer, {
          quality,
          format: 'jpeg',
          generateSizes: shouldGenerateSizes,
        });
      ...
      // Upload all sizes to S3 (JPEG versions)
      const jpegUploadResults = await Promise.all([
      ...
      const [originalUpload, largeUpload, mediumUpload, thumbnailUpload] = jpegUploadResults;

      // Upload WebP versions if available
      let webpUploadResults: any = null;
      if (webpSizes) {
        try {
          webpUploadResults = await Promise.all([
            s3Service.uploadImage({
              folder: 'properties',
              entityId: propertyId,
              filename: file.originalname.replace(/\.(jpg|jpeg|png)$/i, '.webp'),
              buffer: webpSizes.large,
              mimetype: 'image/webp',
              size: 'large',
            }),
            s3Service.uploadImage({
            ...
            s3Service.uploadImage({
              ...
              entityId: propertyId,
              filename: file.originalname.replace(/\.(jpg|jpeg|png)$/i, '.webp'),
              buffer: webpSizes.thumbnail,
              mimetype: 'image/webp',
              size: 'thumbnail',
            }),
          ]);
        } catch (webpUploadError) {
          // Log error but continue with JPEG only
          console.warn('WebP upload failed, continuing with JPEG only:', webpUploadError);
          webpUploadResults = null;
        }
      }
      ...
      // Save to database with S3 URLs (including WebP URLs if available)
      const propertyImage = await PropertyImage.create({
        ...
        thumbnail_webp_url: webpUploadResults ? webpUploadResults[2].url : null,
        medium_webp_url: webpUploadResults ? webpUploadResults[1].url : null,
        large_webp_url: webpUploadResults ? webpUploadResults[0].url : null,
  ...
  static async getPropertyImages(propertyId: number): Promise<any[]> {
    try {
      ...
      return images.map(img => ({
        ...
        thumbnailWebpUrl: img.thumbnail_webp_url,
        mediumWebpUrl: img.medium_webp_url,
        largeWebpUrl: img.large_webp_url,
